  ...
  function showGoods(indentData) {
    ...
    for (var i = 0; i < indentData.length; i++) {
      ...
      if (indentData[i].odStatus == '0') {
        //未付款
        str2 = '<div class="same-style">' + '<div class="consignee"></div>' + //商品操作
        '</div>' + '<div class="same-style state">待付款</div>' + //订单状态
        ...
        //已付款
        str2 = '<div class="same-style">' + '<div class="consignee" data-refund=' + indentData[i].odDelid + '>退款/退货</div>' + //商品操作
        '</div>' + '<div class="same-style state">已支付</div>' + //订单状态
        '<div class="same-style operation-btn">' + //交易操作
        '<div class="handle confirm-receipt" data-confirm=' + indentData[i].odDelid + '>确认收货</div>' + '</div>';
      } else if (indentData[i].odStatus == '2') {
        //2已完成,9
        str2 = '<div class="same-style">' + // '<div class="consignee" data-refund='+ indentData[i].odDelid +'>退款/退货</div>'+  //商品操作
        '</div>' + '<div class="same-style state">已完成</div>' + //订单状态
        '<div class="same-style operation-btn">' + //交易操作
        ...
        //已取消(取消的订单)
        str2 = '<div class="same-style">' + '<div class="consignee">再次购买</div>' + //商品操作
        '</div>' + '<div class="same-style state">已取消</div>' + //订单状态
        '<div class="same-style operation-btn">' + //交易操作
        '</div>';
      } else if (indentData[i].odStatus == '4') {
        //退款成功
        str2 = '<div class="same-style">' + '<div class="consignee"></div>' + //商品操作
        '</div>' + '<div class="same-style state">退款成功</div>' + //订单状态
        '<div class="same-style operation-btn">' + //交易操作
        '</div>';
      } else if (indentData[i].odStatus == '5') {
        //待退款
        str2 = '<div class="same-style">' + '<div class="consignee"></div>' + //商品操作
        '</div>' + '<div class="same-style state">待退款</div>' + //订单状态
        '<div class="same-style operation-btn">' + //交易操作
        '<div class="handle cancel-return" data-confirm=' + indentData[i].odDelid + '>取消退货</div>' + '</div>';
      } else if (indentData[i].odStatus == '6') {
        //退款失败
        str2 = '<div class="same-style">' + '<div class="consignee"></div>' + //商品操作
        '</div>' + '<div class="same-style state">退款失败</div>' + //订单状态
        '<div class="same-style operation-btn">' + //交易操作
        '</div>';
      }
